%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Generate SO(3) irreducible representatives for a large number of
clear all; clc;

top_dir = get_top_dir();
util_dir = [top_dir, 'Util_functions/'];
addpath(genpath(util_dir));

num = 100;
wm1 = linspace(0,pi,num);
th1 = linspace(0,pi,num);
ph1 = linspace(0,2*pi,2*num);

[wm2, th2, ph2] = meshgrid(wm1, th1, ph1);

wm = wm2(:); th = th2(:); ph = ph2(:);
n_angs = size(wm,1);

q0 = cos(wm);
q1 = sin(wm).*sin(th).*cos(ph);
q2 = sin(wm).*sin(th).*sin(ph);
q3 = sin(wm).*cos(th);

a1 = q0-1i*q3;
lb1 = -q2-1i*q1;

a_val = 2;
for alp_val = -a_val:a_val
    for al_val = -a_val:a_val
%         alp_val = -1;al_val = 1;
        U_val = rotation_wo_svd(a1,lb1,q0, [q1, q2, q3], a_val, alp_val, al_val);
        
        mat_name = ['Umat_',num2str(a_val),'_disc_',...
            num2str(num),'_ri_',num2str(alp_val),'_cj_',num2str(al_val),'.mat'];
        
        save(mat_name,'U_val');
    end
end

% av = a_val;
% U_mat = zeros(n_angs, 2*av+1, 2*av+1);
% for ct2=1:n_angs
%     w_1 = wm(ct2);
%     th_1 = th(ct2);
%     ph_1 = ph(ct2);
%     U_mat(ct2,:,:) = rotation_mat(av, [w_1, th_1, ph_1]);
%     % Uw_mat(ct2,:,:) = w_1; Ut_mat(ct2,:,:) = th_1; Up_mat(ct2,:,:) = ph_1;
% end
% ri = -alp_val + a_val + 1;
% cj = -al_val + a_val + 1;


% du1 = [top_dir,'data_files/Umat_grid/Umat_disc_',num2str(num),'/'];




% a = 0:2;
% num_a = size(a,2);
%
% for ct1 =1:num_a
% % ct1 = 4;
% av = a(ct1);
% av
% U_mat = zeros(n_angs, 2*av+1, 2*av+1);
% % Uw_mat = zeros(n_angs, 2*av+1, 2*av+1);
% % Ut_mat = zeros(n_angs, 2*av+1, 2*av+1);
% % Up_mat = zeros(n_angs, 2*av+1, 2*av+1);
% for ct2=1:n_angs
%     w_1 = wm(ct2);
%     th_1 = th(ct2);
%     ph_1 = ph(ct2);
%     U_mat(ct2,:,:) = rotation_mat(av, [-w_1, th_1, ph_1]);
%     % Uw_mat(ct2,:,:) = w_1; Ut_mat(ct2,:,:) = th_1; Up_mat(ct2,:,:) = ph_1;
% end
% mat_name = ['Umat_',num2str(av),'_disc_',...
%     num2str(num),'.mat'];
% % mat_name = [du1,'Umat_',num2str(av),'_disc_',...
% %     num2str(num),'.mat'];
% save(mat_name,'U_mat','-v7.3');
% end

rmpath(genpath(util_dir));